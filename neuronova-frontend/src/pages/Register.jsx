...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Try to log in immediately. In dev (no SMTP), the backend auto-verifies, so this works.
      // In prod, this will fail with "email not verified" and the user is sent to the verify-pending page.
      try {
        await login(formData.email, formData.password);
        navigate('/', { replace: true });
      } catch {
        navigate('/auth/verified', { state: { message: 'Account created. Please check your email to verify your account before logging in.' }});
      }
    } catch (err) {
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#F8F9FA] relative py-[64px]">
      ...
      <div className="relative z-10 w-full max-w-[480px] px-6">
          ...
          <div className="bg-[#F9FAFB] p-[32px] shadow-sm rounded-none border border-transparent relative z-10">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-1">
                <label className="text-[14px] font-medium text-[#4B5563]">First and last name (required)</label>
                <input 
                  type="text" 
                  required
              ...
              <div className="flex flex-col gap-1">
                <label className="text-[14px] font-medium text-[#4B5563]">Work email (required)</label>
                <input 
                  type="email" 
                  required
              ...
              <div className="flex flex-col gap-1">
                <label className="text-[14px] font-medium text-[#4B5563]">Password (required)</label>
                <input 
                  type="password" 
                  required
                  placeholder="••••••••••••"
        ...
        <div className="mt-8 text-center">
          <span className="text-[14px] text-[#4B5563]">Returning user? </span>
          <Link to="/login" className="text-[14px] text-[#4B5563] underline hover:text-[#1E3A5F] transition-colors">
            Log in
          </Link>
        </div>
